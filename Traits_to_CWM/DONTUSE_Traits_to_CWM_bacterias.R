...
Trait_bacterias <- read_csv("/Users/Margot/Desktop/Research/Senckenberg/Project_Ecosystem_strat/Analysis/Data/Raw_data/Bacteria/Bacteria_traits_order.csv")
...
traits_all <- c("metabolism.value", "is.n.pathways.value", "is.motility.value", "d2_up.mean","d1_up.mean", "doubling_h.mean", "sporulation.value")
...
CC_order <- check_coverage(Trait_order, Abundance_order, traits_all, "Std_Order", "Std_Order")
CWM_order <- my_cwm(Trait_bacterias, Abundance_order, traits_all, "Std_Order", "Std_Order")



CWM_aggr = CWM_order[, lapply(.SD, mean), by = Plot, .SDcols = colnames(CWM_order)[3:16]]
