...
seed = 101
#source('~/Desktop/Research/Senckenberg/Project_Ecosystem_strat/Analysis/Code/CWM_analysis/Prep_notebook.R')
...
Table_use = Hypo_table[,.SD, .SDcols = c("Trait_short", "Expected_direction", "Direct/indirect", "Disturbance",	"ref_dist",	"Resources",	"ref_res", 'Est','CI','Padj', 'Fit_exp','Mowing', 'Grazing', 'Fertil', 'Est_nocorr', 'CI_nocorr', 'Fit_exp_nocorr'
                                         )]
...
data_lui <- fread("Data/Environment_data/LUI_standardized_global.txt") 
...
env_from_gaetane <- data.table(read_excel("Data/Environment_data/31018_5_Dataset/31018_5_data.xslx"))
env_from_gaetane[, Plot := EP_PlotID]
env_data_lui = merge.data.table(data_lui, env_from_gaetane[, c('Plot', 'LII',   "Soil.pH", "Soil.depth", "Soil.clay.content" , "Soil.sand.content", "TWI" , 'Grassland.1000')])
      Temp = fread("Data/Environment_data/Ta_200_2008_2018_3a7aa69b37636254/plots.csv")
      Precip = fread("Data/Environment_data/precipitation_radolan_95a5c5f55a798133/plots.csv")
...
list_groups = c('Mites_all', 'Coll_all', 
                ...
                'Plants_BG_no_tree', 'Plants_AG_no_tree', 
...
for (group in list_groups){
  ...
  # Load dataset
  if (weight == FALSE){
    CWM = fread(paste0('Data/CWM_data/CWM_',group, '_noweight.csv'))
  ...
  # Aggregate by plot
if (length(unique(CWM$Year))>1){
CWM_agg = CWM[(Year <= 2019 | is.na(Year)) & grepl('G',Plot), lapply(.SD, mean, na.rm = T), .SDcols = colnames(CWM)[!(colnames(CWM) %in% c('Year', 'V1', 'Plot'))], by = 'Plot']
  print(paste(group, ': ', nrow(CWM_agg), ' plots available for use', sep = ''))}  else {
    CWM_agg = CWM[ grepl('G',Plot), lapply(.SD, mean, na.rm = T), .SDcols = colnames(CWM)[!(colnames(CWM) %in% c('Year', 'V1', 'Plot'))], by = 'Plot']
...
CWM_agg_corr = corrections(CWM_miced[, .SD, .SDcols = colnames(CWM_agg)],  Env_data = env_data_lui, env_corr = env_corr, env_variables = env_var)
...
all_cwm_melt = melt.data.table(all_cwm, id.vars = c('Plot', 'Region', env_var, 'Grazing', 'Mowing', 'Fertil', 'Fertilisation', 'Disturbance','LUI'))
all_cwm_melt_uncorr = melt.data.table(all_cwm_uncorr, id.vars = c('Plot', 'Region',env_var, 'Grazing', 'Mowing', 'Fertil','Fertilisation', 'Disturbance', 'LUI'))
...
# scale everything
all_cwm_melt_all[, c(env_var#[env_var != 'Region']
                     , 'Grazing', 'Mowing', 'Fertil','Fertilisation', 'Disturbance', 'LUI',  'value', 'value_uncorr') := lapply(.SD, function(x){as.numeric(scale(x))}), .SDcols = c(env_var#[env_var != 'Region']
                                                                                                                                                                                     , 'Grazing', 'Mowing', 'Fertil','Fertilisation', 'Disturbance', 'LUI', 'value', 'value_uncorr'), by = variable]

LUI_component_effect = all_cwm_melt_all[, calculate_coeff(.SD), by = variable, .SDcols = c('Grazing', 'Mowing','Fertil', 'Fertilisation', 'Disturbance','value', 'value_uncorr', 'LUI')]
...
# Adding values
Hypothesis_table[Group != '', c("P", "Est", "CI", 'Mowing', 'Grazing', 'Fertil', 'P_uncorr', 'Est_nocorr', 'CI_nocorr') :=
  list(
    round(LUI_component_effect[variable == Trait_short, p], 5),
    round(LUI_component_effect[variable == Trait_short, estimate], 2),
    LUI_component_effect[variable == Trait_short, ci],
    ...
    LUI_component_effect[variable == Trait_short, ci_uncorr]), by = nrows]
...
run_group_pca = function(cwm, traits, trait_direction, direction = '>0', plot = TRUE, env_corr, Labels = NA, annot = NA, naxes = 2){
   ...
   print(quanti.coord)
    ...
    for (i in 1:2) { 
       if (sum(pca_stand$li[, i] == -pca$li[, i]) != 0) { # If the direction of the pca was changed then we also change the direction of the supcol
...
#colnames(var) <- c("x", "y")
#colnames(ind) <- c("x", "y")
#colnames(quanti.coord)[c(1,2)] <- c("x", "y")
...
#  xlim(range(c(1.6*var$x*r*0.7, ind$x, quanti.coord$x*r*0.7))) +
#  ylim(range(c(1.6*var$y*r*0.7, ind$y, quanti.coord$y*r*0.7))) +
#  geom_point() + theme_bw() +
...
#  geom_text(data = var*r*0.7, aes(x = 1.3*x, y = 1.2*y, label = Labels[rownames(var)]), inherit.aes = F, color = #'steelblue') + 
#  geom_segment(data = quanti.coord, aes(x = 0, y = 0, xend = x*r*0.7, yend = y*r*0.7), inherit.aes = F, color = 'darkred',
#               arrow = arrow(length = unit(0.1, "inches"))) +
#  #    annotate(geom = 'label', x = -Inf, y =+Inf, label = Criteria_label,
#  #         hjust = -0.1, vjust = 1.5) +
#  geom_text(data = quanti.coord, aes(x = 1.3*x*r*0.7, y = 1.2* y*r*0.7, label = Lab), inherit.aes = F, color = 'darkred')
 ...
 direction_axis = numeric()
  for (i in 1:length(direction)){
   if (pca_stand$var$coord[trait_direction[i], i] > 0 & direction[i]  == '>0' |
    ...
    corr_unsignificant = 0
    for (trait in traits){
     cor = cor.test(unlist(cwm[, ..trait]), pca_stand$ind$coord[,1])
    if (cor$p.value > 0.05){
      corr_unsignificant = corr_unsignificant+1
    }
   }
   ...
   keep_criteria[, enough_support := More_than_random >2 & Prop.traits.40 >= 0.60]
   keep_criteria[, partial := More_than_random >2 & Prop.traits.25 >= 0.60 & Cor_unsignificant <= 1]
   keep_criteria[, LUI.Axis1 := round(cor.test(cwm$LUI, pca_stand$ind$coord[,1])$p.value, 3)]
   keep_criteria[, LUI.Axis2 := round(cor.test(cwm$LUI, pca_stand$ind$coord[,1])$p.value, 3)]
...
rownames(pca_stand$var$coord) = rownames(pca_stand$var$cor) = rownames(pca_stand$var$cos2) = Labels

if (env_corr == TRUE){
  plot_pca_12 = fviz_pca(pca_stand, title = '', geom = 'point', geom.var = c("arrow", 'text'), col.quanti.sup = 'darkred', col.var = 'grey30', 
                       labelsize = 3, repel = T)
}
if (env_corr == FALSE){
  plot_pca_12 = fviz_pca(pca_stand, title = '', geom = 'point', geom.var = c("arrow", 'text'), col.quanti.sup = 'darkred', col.var = 'grey30', habillage = factor(substr(rownames(pca_stand$ind$coord), 1, 1)), 
                       labelsize = 3, repel = T) +
  scale_shape_discrete( breaks = c('A', 'H', 'S'), labels = c('South', 'Central', 'North'), name = 'Region')+
  scale_color_viridis(discrete = TRUE, begin = 0.3, breaks = c('A', 'H', 'S'), labels = c('South', 'Central', 'North'), name = 'Region')
...
pca_plants_all = run_group_pca2(env_corr = env_corr, all_cwm_miced, all_plant_traits , 'SLA')
pca_plants_all$plot12
pca_plants_all$criteria

ggsave(plot = pca_plants_all$plot12,file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Plant_com_pca.pdf', sep = ''), width = 6, height = 6)
...
pca_mic = run_group_pca2(env_corr = env_corr, all_cwm_miced, traits_Microbes, 'mic_Fpathotroph',  Labels = labels_Microbes)
pca_mic$plot12
pca_mic$criteria

ggsave(plot = pca_mic$plot12, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Microbes_com_pca.pdf', sep = ''), width = 6, height = 6)
```
...
pca_arthro_herb = run_group_pca2(env_corr = env_corr, all_cwm_miced, traits_Arthropods_above_herb, 'Ah_Dispersal', Labels = labels_arthro_herb)
pca_arthro_herb$plot12
ggsave(plot = pca_arthro_herb$plot12, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Arthro_herb_AG_com_pca.pdf', sep = ''), width = 6, height = 6)
...
pca_arthro_herb_below = run_group_pca2(env_corr = env_corr, all_cwm_miced[complete.cases(all_cwm_miced[,..traits_Arthropods_below_herb])], traits_Arthropods_below_herb, 'Ah_b_Dispersal', Labels = labels_arthro_below_herb)
...
ggsave(plot = pca_arthro_herb_below$plot12,file =
         paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Arthro_herb_BG_com_pca.pdf', sep = ''), width = 6, height = 6)
...
pca_but = run_group_pca2(env_corr = env_corr, all_cwm_miced[complete.cases(all_cwm_miced[, ..traits_Butterflies])], Labels = labels_but, traits_Butterflies, 'but_flight')
pca_but$plot12
pca_but$criteria

print_table(Hypothesis_table[Group  %in% c('Arthropods (herbivores)', 'Butterflies'),])

ggsave(plot = pca_but$plot12, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Butterflies_com_pca.pdf', sep = ''), width = 6, height = 6)
```
...
pca_arthro_pred = run_group_pca2(env_corr = env_corr, all_cwm_miced, traits_Arthropods_above_omni_carni, c('Aoc_Dispersal', 'Aoc_Dispersal'), direction = c('>0','>0'), Labels = c('Dispersal', 'Body size'))
pca_arthro_pred$plot12

ggsave(plot = pca_arthro_pred$plot12, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Arthro_oc_com_pca.pdf', sep = ''), width = 6, height = 6)

pca_arthro_pred_below = run_group_pca2(env_corr = env_corr, all_cwm_miced, traits_Arthropods_below_omni_carni, 'Aoc_b_Dispersal', Labels = c('Dispersal', 'Body size'))
pca_arthro_pred_below$plot12

ggsave(plot = pca_arthro_pred_below$plot12,file =
         paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Arthro_oc_BG_com_pca.pdf', sep = ''), width = 6, height = 6)
```
...
pca_birds_all= run_group_pca2(env_corr = env_corr, all_cwm_miced[complete.cases(all_cwm_miced[,..traits_Birds_insect])], traits_Birds_insect, 'Bi_TOffsprings', Labels = birds_labels)
pca_birds_all$plot12
pca_birds_all$criteria


ggsave(plot = pca_birds_all$plot12,file =  paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Birds_i_com_pca.pdf', sep = ''), width = 6, height = 6)
...
pca_mites_all = run_group_pca2(env_corr = env_corr, all_cwm_miced[complete.cases(all_cwm_miced[, ..traits_Mites_all])], traits_Mites_all, 'mites_DaysAdult', direction = '<0', Labels = mites_labels)
pca_mites_all$plot12
pca_mites_all$plot13
pca_mites_all$criteria

ggsave(plot = pca_mites_all$plot12,file = 
         paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Mites_com_pca.pdf', sep = ''), width = 6, height = 6)
```
...
pca_coll_all = run_group_pca2(env_corr = env_corr, all_cwm_miced[complete.cases(all_cwm_miced[, ..traits_Coll_all])], traits_Coll_all, 'col_Depth',  Labels = col_labels)
pca_coll_all$plot12
pca_coll_all$plot13
pca_coll_all$criteria

ggsave(plot = pca_coll_all$plot12, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Coll_com_pca.pdf', sep = ''), width = 6, height = 6)
...
pca_bats_all = run_group_pca2(env_corr = env_corr, all_cwm_miced[complete.cases(all_cwm_miced[, ..traits_Bats])], traits_Bats, 'bat_offspring', Labels = bats_labels)
pca_bats_all$plot12
pca_bats_all$plot13
pca_bats_all$criteria

ggsave(plot = pca_bats_all$plot12,file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/Bats_com_pca.pdf', sep = ''), width = 6, height = 6)
```
...
list.df = list(  # Getting all the axes and multiplying the slow-fast axis by )1 if needed so that increasing axis values means faster system
        ...
        data.table(  Dim.1 = all_cwm$P_patho, rn =all_cwm$Plot),
        data.table( Dim.1 = -all_cwm$Pb_Size, rn =all_cwm$Plot),
        data.table( Dim.1 = -all_cwm$Ps_Size, rn =all_cwm$Plot),
...
ggsave(plot = gg_correlation_axes ,file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/gg_correlation_axes.pdf', sep = ''), width = 8, height = 8)
...
dd = Reduce(function(...) merge(..., all = TRUE, by = 'Plot'), 
  list( data.table(plant                = pca_plants_all$PCA$ind$coord[, 1]*pca_plants_all$direction,
        ...
        data.table(Arth_omnicarni= pca_arthro_pred$PCA$ind$coord[,2]*pca_arthro_pred$direction[2]       , 
        ...
        data.table(Microbes = pca_mic$PCA$ind$coord[, 1]*pca_mic$direction, 
                   Plot = rownames(pca_mic$PCA$ind$coord)),
        data.table(Protists_patho =  all_cwm$P_patho,    
                   Plot =  all_cwm$Plot),
        data.table(Protists_bact = -all_cwm$Pb_Size,    
                   Plot = all_cwm$Plot),
        data.table(Protists_sec = -all_cwm$Ps_Size,    
                   Plot = all_cwm$Plot),
...
ggsave(plot = pca_pca_plot ,file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/All_groups_pca.pdf', sep = ''), width = 8, height = 6)

ggsave(plot = pca_pca_plot_LUI, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/All_groups_pca_LUI_nolabel.pdf', sep = ''), width = 8, height = 6)

ggsave(plot = pca_pca_plot_nolabel, file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/All_groups_pca_nolabel.pdf', sep = ''), width = 8, height = 6)
...
# View(data.table(modificationindices(model_init_AG))[order(mi, decreasing = T), ])
 ...
 model_AG = sem(paste(autowrite_SEM_full(model_init_AG, Trophic_levels), 'Bats ~~ BirdsI', sep = '\n'),
                ...
                estimator = "ML",
                missing = "ML",
                se = 'bootstrap', bootstrap = 10#00
                )
 
 semtable_AG <- semTable (model_AG, fits = c('pvalue', 'rmsea', 'cfi', 'bic'), print.results = FALSE, type = 'csv',paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_table_AG.csv', sep = ''))
...
c(0,          1.5,    1.5,     3,              4.5,   4.5,      0 )), ncol = 2)
 ...
 pdf(file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_AG.pdf', sep = ''), width = 5)
 plot(p)
 dev.off()
```
...
model_BG = sem(paste(autowrite_SEM_full(model_BG_init, Trophic_levels), 'Protists_patho ~~ Microbes \n Protists_patho ~~ 0*Arth_omnicarni_below', sep = '\n'),
 ...
 semtable_BG <- semTable (model_BG, fits = c('pvalue', 'rmsea', 'cfi', 'bic'), print.results = FALSE, type = 'csv',paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_table_BG.csv', sep = ''))
...
c(0, -1.5,  -1.5,   -3,      -3,       -3,   -3,    - 4.5,        0)), ncol = 2)
 ...
 pdf(file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_BG.pdf', sep = ''), width = 5)
 plot(p)
 dev.off()
```
...
gg_fullSEM
 ...
 ggsave(gg_fullSEM, file = 
           paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_All.pdf', sep = ''), width = 8, height = 11)
...
gg_effects
ggsave(gg_effects, file =  paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/gg_SEM_effects.pdf', sep = ''), width = 5, height = 12)
...
ggsave(gg_individual_groups, file =  paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/gg_individual_groups.pdf', sep = ''), width = 5, height = 12)
...
additional_info <- fread('/Users/Margot/Desktop/Research/Senckenberg/Data/Functions/27087_21_Dataset/synthesis_grassland_function_metadata_ID27087.csv')
...
original_synth_func <- fread('/Users/Margot/Desktop/Research/Senckenberg/Data/Functions/27087_21_Dataset/27087_21_data.csv')
...
EF_clean_corr = corrections(EF_clean_mice,  Env_data = env_data_lui,  env_corr = env_corr, env_variables = env_var)
...
fun_pca = fviz_pca(pca_mf, title = '', geom = 'point', geom.var = c("arrow"), habillage = factor(substr(env_data_lui$Region, 1, 1)), col.quanti.sup = 'darkred', col.var = 'grey30')+
 ...
 pdf(file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/PCA_functions.pdf', sep = ''), width = 6, height = 4)
fun_pca
 dev.off()
 ...
 pdf(file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/PCA_functions_wtext.pdf', sep = ''), width = 6, height = 4)
fun_pca
 dev.off()
...
pca_microbes = data.frame(pca_mic$PCA$ind$coord)
...
mod_mic = step(lm(Dim.1_fun~ Dim.1_mic , data = dat), trace = -1)
...
model_table = data.frame(matrix(c(
                   ...
                   'Functions slow-fast ~ taxonomic multidiversity', output_model(mod_div)), ncol = 4, byrow = T))
...
layout_fun = matrix(c(c(2, 1, 0),
 ...
 p =  semPaths(mod_function,  what = "std",layout = layout_fun, 
  ...
  semtable_lui <- semTable(mod_function, fits = c('pvalue', 'rmsea', 'cfi', 'bic'), print.results = FALSE, type = 'csv',paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_table_lui.csv', sep = ''))
  
  
 png(file = paste('/Users/Margot/Desktop/Research/Senckenberg/Documents/Papers/Traits/Figures/Envcorr_', env_corr, '/weight_', weight,'/SEM_functions.png', sep = ''), width = 600, height = 400)
 plot(p)
 dev.off()
