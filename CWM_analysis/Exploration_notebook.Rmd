...
# Checking NAs: below-ground arthropods have too many NAs so we won't use it
 apply(dd, 2, function(x){length(x[is.na(x)])})

# We are removing plots for which there is more than one NA
#dd$n.na = apply(dd, 1, function(x){length(x[is.na(x)])})

#dd = dd[n.na < 2,]
  
# Then we mice it
dd[,  c("plant", "Arth_herb", "Arth_herb_below",
                           "Arth_omnicarni", 
                           "Arth_omnicarni_below","Microbes",  "Protists_patho",  "Protists_bact", 
                           "Protists_sec",  "Mites", "Coll", "But",   "BirdsI",  "Bats")] = mice::complete(mice(dd[,  c("plant", "Arth_herb", "Arth_herb_below",
                           "Arth_omnicarni", 
                           "Arth_omnicarni_below","Microbes",  "Protists_patho",  "Protists_bact", 
                           "Protists_sec",  "Mites", "Coll", "But",   "BirdsI",  "Bats")], printFlag = FALSE, method = 'mean'))


PCA_pca = dudi.pca(dd[,  c("plant", "Arth_herb", #"Arth_herb_below",
                           "Arth_omnicarni", 
                           "Arth_omnicarni_below","Microbes",  "Protists_patho",  "Protists_bact", 
...
# Remove non-significant with standardised effects < 0.1
mod_AG =  "
        plant ~ LUI 
        ...
        Arth_omnicarni ~ LUI + plant + But+Arth_herb
        BirdsI ~ LUI + Arth_herb + plant + But 
        Bats ~   LUI + Arth_herb + plant + But 
"
...
#est = data.table(parameterestimates(model_AG, boot.ci.type = 'bca.simple'))
...
mod_BG =  "
        ...
        Mites  ~          LUI + plant + Microbes + Protists_sec + Protists_patho
        Coll   ~          LUI + Microbes + plant + Protists_sec

        Arth_omnicarni_below ~ LUI + Mites + Coll  + Protists_sec  
"
...
c(4,    3.5,  4.5,   5,     3,      2,     4,    6,   4,    -2),
c(0, -1.5,  -1.5,   -3,      -3,       -3,   -3,  -3,  - 4.5,   0)), ncol = 2)
...
#c(-1,    -2.5,    -3,     -4,      -3.5,    6,   -4,      -6,          0,     -2)
 ...
 est = data.table(parameterestimates(model_BG, boot.ci.type = 'bca.simple'))
#View(est[op == ':=', list(label, est = round(est, 2), p = round(pvalue, 4))])
