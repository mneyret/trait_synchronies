...
setwd('/Users/Margot/Desktop/Research/Senckenberg/Project_Ecosystem_strat/Analysis/Data/Raw_data/Plants/Try_request_10498')
TRYdata1 <- fread("Input_Data/10498.txt", header = T, sep = "\t", dec = ".", quote = "", data.table = T)
...
# work_data <- work_data[, .SD, .SDcols = c(1:4, (nc+1),(nc+2),(nc+3),(nc+4), 5:nc)]
...
table(work_data$DataName, work_data$TraitName)
sapply(unique(work_data$TraitName), function(x){
  tab = table(work_data[work_data$TraitName == x,]$DataName)
  return(tab[tab>0])
})
...
exclude <- subset(work_data, 	
                (TraitID == 3116  & UnitName != "mm2 mg-1") |
                (TraitID == 3116  & UnitName != "mm2 mg-1") |
								(TraitID == 47    & UnitName != "g g-1")    |
								(TraitID == 14    & UnitName != "mg/g")     |
								(TraitID == 15    & UnitName != "mg/g")     |
								(TraitID == 7     & UnitName != "")         |
								(TraitID == 3106  & UnitName != "m")        |
								(TraitID == 26    & UnitName != "mg")       |
