...
#  }
#    if (weight == TRUE){
#    CWM = fread(paste0('Data/CWM_data/CWM_',group, '.csv'))
#
#  }
#  # Aggregate by plot
...
#}
```
# Check intra v. inter plot variation in LUI and CWM for groups sampled multiple times
...
varpart_results = data.table(Group='LUI', Plot = round(modLUI$part$fract$Adj.R.squared[1], 2), 
                                          Year = round(modLUI$part$fract$Adj.R.squared[2], 2), 
                                     Residuals = round(1-modLUI$part$fract$Adj.R.squared[3], 2))
...
test = mice::complete(mice(dcast(all_cwm_raw[Group == group,], Plot + Year ~ traitName, value.var = 'traitValue'), seed=101))
mod <- varpart(test[, -c(1,2)], factor(test$Plot), factor(test$Year))

varpart_results = rbind(varpart_results,
                        data.table(Group=group, 
                                   Plot = round(mod$part$fract$Adj.R.squared[1], 2), 
                                   Year = round(mod$part$fract$Adj.R.squared[2], 2), 
                                  Residuals = round(1-mod$part$fract$Adj.R.squared[3], 2)))
...
#all_cwm_melt = melt.data.table(all_cwm_test, id.vars = c('Plot', 'Region', env_var, 'Grazing', 'Mowing', 'Fertil', #'Fertilisation', 'Disturbance','LUI'))
...
# scale everything
#all_cwm_test = copy(all_cwm_lui)
#all_cwm_test[, c(env_var#[env_var != 'Region']
#                     , 'Grazing', 'Mowing', 'Fertil','Fertilisation', 'Disturbance', 'LUI',  'value'#, 'value_uncorr'
#                 ) := lapply(.SD, function(x){as.numeric(scale(x))}), .SDcols = c(env_var#[env_var != 'Region']
...
paste(paste(new_col, names(new_col), sep = ': '), collapse = ' ')
...
list.df = list(  # Getting all the axes and multiplying the slow-fast axis by -1 if needed so that increasing axis values means faster system
        ...
        data.table( Dim.1 = -all_cwm$protists_bact_Size, rn =all_cwm$Plot),
        data.table( Dim.1 = -all_cwm$protists_sec_cons_Size, rn =all_cwm$Plot),
...
```{r Draw fig1bis, echo = FALSE, eval = TRUE}
Hypothesis_table[, Group_f := Group]
#Hypothesis_table[grepl('protist', Group_f), Group_f := 'protists']
...
plot_traits = ggplot(Hypothesis_table, aes(x = reorder(labels, -Est), y = Est, ymin = CI_low, ymax = CI_high, color = Fit_expectations)) +
  geom_point(size = 3) + geom_errorbar(width = 0.4, size = 1.5) + 
...
#
#all_cwm_melt[, trait_type := traitName]
#all_cwm_melt[grepl('Dispersal', trait_type), trait_type := 'Dispersal']
#all_cwm_melt[grepl('Size', trait_type) | grepl('[Mm]ass', trait_type) | grepl('bact_fungi_logVolume', trait_type), trait_type := 'Size']
#all_cwm_melt[grepl('[Gg]eneralism', trait_type) | grepl('mites_Feeding_spec', trait_type), trait_type := 'Generalism']
#all_cwm_melt[grepl('[Vv]oltinism', trait_type) | grepl('Gen_per_year', trait_type)| grepl('[Gg]enerations', trait_type), trait_type := #'Generations']
#all_cwm_melt[grepl('bats_Lifespan', trait_type) | grepl('birds_GenLength', trait_type)| grepl('mites_DaystoAdult', trait_type), trait_type #:= 'Lifespan']
#all_cwm_melt[grepl('bats_Number_offspring', trait_type) | grepl('birds_logIncub_time', trait_type)| grepl('birds_logBrood', #trait_type)| grepl('coll_Repro_sex', trait_type)| grepl('mites_Repro_sex', trait_type), trait_type := 'Reproduction']
#all_cwm_melt[, unique(trait_type)]
...
gg_fullSEM =  ggraph(param_graph, layout = layout_all)+
    ...
    theme_graph(base_size = 1, base_family = 'Helvetica') +theme(legend.position = "none") +
...
pca_all_traits = data.frame(pca_all$ind$coord)
colnames(pca_all_traits) = paste(colnames(pca_all_traits), '_allTraits', sep = '')
pca_all_traits$Plot = dd$Plo
...
#
  ...
  semtable_lui <- semTable(mod_function, fits = c('pvalue', 'rmsea', 'cfi', 'bic'), print.results = FALSE, type = 'csv',paste('Results/Envcorr_', env_corr, '/weight_', weight,'/SEM_table_lui.csv', sep = ''))
...
summary(mod_function_allTraits)
parameterestimates(mod_function_allTraits, rsquare = TRUE)
...
#
  ...
  semtable_lui <- semTable(mod_function_allTraits, fits = c('pvalue', 'rmsea', 'cfi', 'bic'), print.results = FALSE, type = 'csv',paste('Results/Envcorr_', env_corr, '/weight_', weight,'/SEM_table_lui_all_traits.csv', sep = ''))
